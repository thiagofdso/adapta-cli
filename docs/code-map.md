...
CLI -> resolução de `--config` ou `ADAPTA_DEBATE_CONFIG` ou perguntas interativas -> resolução do tema por `--prompt` ou `--prompt-file` -> parsing opcional de `--file` -> `debate_service` -> upload único opcional no cliente Adapta -> chats separados por agente ao longo das rodadas, com rodadas normais em paralelo por agente -> conclusão final -> stdout e/ou arquivo -> limpeza remota dos chats
...
CLI -> resolução de `--input`/`--output`, `--input`/`--output-dir` ou `--input-dir`/`--output-dir` -> `destilador_service` -> upload do PDF ao Adapta -> geração de 7 dimensões com prompts internalizados -> paralelismo por item quando há múltiplos documentos -> consolidação markdown -> stdout e/ou arquivo -> preservação opcional de parciais -> limpeza best-effort dos artefatos
